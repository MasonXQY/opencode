...
FactorySight Zen 是由 OpenCode 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 OpenCode 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果您是新使用者，我們建議從 FactorySight Zen 開始。
:::

1. 在 TUI 中執行 `/connect` 指令，選擇 `FactorySight Zen`，然後前往 [opencode.ai/auth](https://opencode.ai/zen)。
...
FactorySight Zen 是由 OpenCode 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。

1. 登入 **<a href={console}>FactorySight Zen</a>** 並點擊 **Create API Key**。

2. 執行 `/connect` 指令並搜尋 **FactorySight Zen**。
