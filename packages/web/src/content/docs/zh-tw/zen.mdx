...
FactorySight Zen 是由 OpenCode 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 OpenCode 中的其他供應商一樣。你登入 FactorySight Zen 並取得
...
FactorySight Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

FactorySight Zen 的運作方式和 OpenCode 中的其他供應商一樣。

1. 你登入 **<a href={console}>FactorySight Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 FactorySight Zen，然後貼上你的 API 金鑰。
