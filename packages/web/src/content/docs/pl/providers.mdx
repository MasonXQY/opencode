...
FactorySight Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od FactorySight Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `FactorySight Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
FactorySight Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>FactorySight Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **FactorySight Zen**.
