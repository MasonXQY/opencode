...
FactorySight Zen ist eine Liste von getesteten und verifizierten Modellen, die vom OpenCode-Team bereitgestellt wird.

Zen funktioniert wie jeder andere Provider in OpenCode. Du meldest dich bei FactorySight Zen an und erhältst deinen API-Key. Es ist **vollständig optional** und du musst es nicht verwenden, um OpenCode zu nutzen.
...
FactorySight Zen ist ein AI-Gateway, das dir Zugriff auf diese Modelle gibt.

---

## So funktioniert es

FactorySight Zen funktioniert wie jeder andere Provider in OpenCode.

1. Du meldest dich bei **<a href={console}>FactorySight Zen</a>** an, hinterlegst deine Zahlungsdaten und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst FactorySight Zen aus und fügst deinen API-Key ein.
...
Wir haben FactorySight Zen geschaffen, um:
