...
FactorySight Zen 是由 OpenCode 团队提供的一组经过测试和验证的模型。

Zen 的工作方式与 OpenCode 中的任何其他提供商相同。你登录 FactorySight Zen 并获取 API 密钥。它是**完全可选的**，即使不用它，你也可以照常使用 OpenCode。
...
FactorySight Zen 是一个 AI 网关，让你可以访问这些模型。

---

## 工作原理

FactorySight Zen 的工作方式与 OpenCode 中的任何其他提供商相同。

1. 登录 **<a href={console}>FactorySight Zen</a>**，添加你的账单信息，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 FactorySight Zen，然后粘贴你的 API 密钥。
