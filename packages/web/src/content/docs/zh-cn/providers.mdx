...
FactorySight Zen 是由 OpenCode 团队提供的模型列表，这些模型已经过测试和验证，能够与 OpenCode 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果你是新用户，我们建议从 FactorySight Zen 开始。
:::
...
FactorySight Zen 是由 OpenCode 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。

1. 登录 **<a href={console}>FactorySight Zen</a>** 并点击 **Create API Key**。

2. 执行 `/connect` 命令并搜索 **FactorySight Zen**。
