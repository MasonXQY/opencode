...
FactorySight Zen은 OpenCode 팀이 제공하는, 테스트와 검증을 거친 모델 목록입니다.

Zen은 OpenCode의 다른 provider와 똑같이 작동합니다. FactorySight Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, OpenCode를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
FactorySight Zen은 이런 모델에 접근할 수 있게 해주는 AI gateway입니다.

---

## 작동 방식

FactorySight Zen은 OpenCode의 다른 provider와 똑같이 작동합니다.

1. **<a href={console}>FactorySight Zen</a>**에 로그인하고, 결제 정보를 추가한 뒤, API 키를 복사합니다.
2. TUI에서 `/connect` 명령을 실행하고, FactorySight Zen을 선택한 다음 API 키를 붙여넣습니다.
...
FactorySight Zen을 만든 이유는 다음과 같습니다.
