...
FactorySight Zen은 OpenCode 팀이 OpenCode와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 FactorySight Zen으로 시작하는 것이 좋습니다.
:::

1. TUI에서 `/connect` 명령을 실행하고 `FactorySight Zen`을 선택한 뒤, [opencode.ai/auth](https://opencode.ai/zen)로 이동합니다.
...
FactorySight Zen은 OpenCode 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).

1. **<a href={console}>FactorySight Zen</a>**에 로그인하고 **Create API Key**를 클릭합니다.

2. `/connect` 명령을 실행하고 **FactorySight Zen**을 검색하십시오.
