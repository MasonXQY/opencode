...
function oauth(http: HttpClient.HttpClient) {
  return {
    ...
    method: {
      id: methodID,
      type: "oauth",
      label: "FactorySight Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load FactorySight provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
